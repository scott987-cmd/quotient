...
public enum TaskDecomposer {
    ...
    ///
    /// 干活的 agent 有能力一次吃下更大的块。4 步足够把「碰构建配置的部分」
    /// 单独隔出来（那是拆图真正的价值），又不至于把一件事切成八份。
    ...
    ///   完成标准写成了编号清单，`mentionsSteps` 把 "1. " "2. " 当成了步骤。
    ///
    /// 而支持拆图的那条理由（便宜平台干机械部分、贵的只干难的那步）
    /// 在**工作区按仓库×平台复用**落地之后已经站不住了：一个 agent
    /// 带着完整会话干 5 件事，严格优于 5 个 agent 各自重新认路。
    ///
    /// 现在只有两种情况值得拆，两种都是「一次执行装不下」而不是「活多」：
    ///
    /// 1. **碰高危路径** —— 那一步要单独拿给人放行，混在大任务里
    ///    会把整包拖住等审批。
    /// 2. **跨能力** —— 比如先生图再接线：生图执行器改不了代码，
    ///    编码执行器画不了图，一次执行里根本不可能都干。
    public static func shouldDecompose(_ t: WorkTask) -> Bool {
        if GitWorkspace.mentionsRiskyPath(t.prompt) { return true }
        if spansCapabilities(t.prompt) { return true }
        // **写明「第一步…第二步…」的仍然拆。**
        // 那是人明确表达了顺序依赖，不是我们猜的。
        return mentionsSteps(t.prompt)
    }
    ...
    static func mentionsSteps(_ prompt: String) -> Bool {
        // **不认 "1. " "2. " 这类编号。** 它们在中文提示词里绝大多数是
        // 「完成标准」「产出要求」的条目，不是步骤 —— 而一旦误判，
        // 一个单步任务会被拆成七步图，失败面积乘七倍。
        // 「生 4 张遗物图标」就是这么被拆的：编号的是验收条件。
    ...
    static func instruction(for task: WorkTask) -> String {
        ...
        - **最多 \(maxSteps) 步，能两步做完就别拆三步。**
          ...
          只有下面这两种理由值得多分一步:
          ①碰构建/CI/依赖/权限,要人工确认;②真的必须等上一步的产物才能开始。
