...
public enum AskContract {
    ...
    public static func clause(askFile: String) -> String {
        ...
        **人工登录、验证码、双重认证、网页授权或必须由人点击的 GUI** 不是可以
        静默等待的执行步骤。遇到这类硬阻塞，直接运行：

        `llmq work needs-human "需要对方完成的具体操作，以及完成后如何验证"`

        系统会保存你的已有进度、停止本次模型进程、把任务标成“等待你的答复”并
        推送到移动端。不要只在仓库里写 `BLOCKED.md`，那不会通知任何人。
