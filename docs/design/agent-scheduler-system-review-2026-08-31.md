...
状态：已经独立复核批准；阶段 A、B、C、D 已在当前工作区实施并验证，尚未提交、合入或发布。
后续健康/额度阶段仍待实施，不得把 A–D 完成误报为整个调度架构已完成。
...
- 本阶段没有提交、合入或发布；跨机协调、控制面/执行面生命周期隔离、完整异步咨询
  和健康/额度分层不属于本阶段，仍按后续阶段实施。
...
- 反向破坏分别证明测试能拦住 Executor `KeepAlive=true` 和 Watchdog 超时后晚到提交。
  ...
  （2 项按环境跳过，0 失败）。
- 本阶段没有提交、合入、发布或安装；当前正在运行的服务仍不会自动切换到这份工作区代码。
  跨机与完整异步协作属于阶段 D，健康/额度分层属于后续阶段。
...
- 反向破坏把 machineID 去重改回 machineName 后，同名不同机器用量门禁稳定失败；恢复实现后
  ...
  （2 项按环境跳过，0 失败）。
- 本阶段没有提交、合入、发布或安装；当前服务不会自动切换到工作区代码。阶段 E 的
  runner/capability 健康模型与额度事实分层仍待实施。
