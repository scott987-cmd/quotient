...
以 `【媒体】` 开头的任务不派给编码执行器（那会白跑一轮 -- 它只会试图
「写代码生成图片」），而是走 MiniMax 的媒体执行器，把提示词正文里的
资产清单逐行变成真文件。每行一条：

```text
REF   <参考图路径>                        （之后的 IMG 锁定同一角色，REF - 清除）
IMG   <相对路径> [宽高比] :: <画面描述>
VIDEO <相对路径> [秒数4-15] :: <画面描述>   （额度每天仅 3 条，关键画面才用；有 REF 时 = 让参考图动起来）
MUSIC <相对路径> :: <音乐描述>
```
...
【媒体】生成主视觉素材
REF   refs/hero.png
IMG   assets/creature.png :: 深渊里的小型发光生物，青色荧光，剪影感
IMG   assets/zone-2.png 16:9 :: 中层海域背景，体积光从上方打下
VIDEO assets/hero-loop.mp4 8 :: 缓慢推镜从中景推近，荧光粒子上升
MUSIC assets/ambient.mp3 :: 深海环境音，缓慢，压迫感渐强
```

- **REF** 一条管住后面所有 IMG：同一个角色在几十张图里长相一致（纯文生图
  做不到，脸会漂）；`REF -` 清除，之后回到纯文生图。
- **VIDEO** 之前有 REF 生效时走图生视频，等于让参考图动起来。
- 整份清单一次跑完（媒体任务不会被拆分），全部成功才算过；已存在的文件
  直接跳过、不重烧额度，失败重试接着半成品续跑。
- 图片关键字是 IMG，不是 IMAGE -- 写错了就一行都不匹配。
